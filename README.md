...
Конструктор:
`constructor(api: IApi)` - получение объекта для реализации интерфейса IApi.

Поля класса:
`api: IApi` - экземпляр класса для выполнения запросов.

Методы класса:
`getProducts(): Promise<IProductsResponse>` - делает запрос и возвращает объект с количеством товаров и массивом товаров.
`createOrder(order: IOrder): Promise<IOrderResponse>` - делает запрос и возвращает объект, подтверждающий покупку.